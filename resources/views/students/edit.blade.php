...
<script>
    $(document).ready(function() {
        ...
        // Load eligibility qualifications when subcourse is selected
        function loadEligibilityQualifications(subCourseId) {
            if (!subCourseId) {
                return;
            }

            $.get('/students/get-subcourse-details/' + subCourseId + '?student_id={{ $student->id }}', function(data) {
                // If there are eligibility requirements and no existing qualifications, show them
                if (data.eligibility && data.eligibility.length > 0) {
                    // Check if we already have qualifications loaded
                    if ($('.qualification-entry').length === 0) {
                        data.eligibility.forEach(function(qual) {
                            var fieldHtml = `
                                <div class="row mb-4 p-3 border rounded qualification-entry">
                ...
                toastr.error("Failed to load eligibility details.");
            });
        }
        ...
        function loadSubCourseDetails(subCourseId) {
            ...
            $.get('/students/get-subcourse-details/' + subCourseId + '?student_id={{ $student->id }}', function(data) {
                ...
                // Load eligibility qualifications if no existing qualifications are present
                if ($('.qualification-entry').length === 0 && data.eligibility && data.eligibility.length > 0) {
                    loadEligibilityQualifications(subCourseId);
                }
            }).fail(function() {
                toastr.error("Failed to load sub course details.");
            });
