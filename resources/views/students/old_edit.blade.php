        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form id="student-form" action="{{ route('students.update', $student->id) }}" method="POST"
                    ...
                    <h5 class="mt-4 mb-3 text-secondary"><i class="bi bi-book me-2"></i>Previous Education /
                        Qualifications</h5>
                    <div id="prev-education-container">
                        @php
                        $prevQualifications = old('prev_qualification',
                        $studentQualifications->pluck('qualification')->toArray() ?? []);
                        $prevBoards = old('prev_board', $studentQualifications->pluck('board')->toArray() ?? []);
                        $prevYears = old('prev_passing_year', $studentQualifications->pluck('passing_year')->toArray() ?? []);
                        $prevMarks = old('prev_marks', $studentQualifications->pluck('marks')->toArray() ?? []);
                        $prevResults = old('prev_result', $studentQualifications->pluck('result')->toArray() ?? []);
                        $prevDocuments = old('prev_document', $studentQualifications->pluck('document')->toArray() ??
                        []);
                        @endphp

                        @if(count($prevQualifications) > 0)
                        @foreach($prevQualifications as $index => $qual)
                        <div class="row mb-4 p-3 border rounded">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="prev_qualification[]" class="form-control" value="{{ $qual }}"
                                    readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Board / University</label>
                                <input type="text" name="prev_board[]" class="form-control"
                                    value="{{ $prevBoards[$index] ?? '' }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Passing Year</label>
                                <input type="text" name="prev_passing_year[]" class="form-control"
                                    value="{{ $prevYears[$index] ?? '' }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Obtained Marks</label>
                                <input type="text" name="prev_marks[]" class="form-control"
                                    value="{{ $prevMarks[$index] ?? '' }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Result</label>
                                <select name="prev_result[]" class="form-select">
                                    <option value="">-- Select Result --</option>
                                    <option value="Pass" {{ ($prevResults[$index] ?? '' )=='Pass' ? 'selected' : '' }}>
                                        Pass</option>
                                    <option value="Fail" {{ ($prevResults[$index] ?? '' )=='Fail' ? 'selected' : '' }}>
                                        Fail</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Upload Marksheet / Certificate <small
                                        class="text-muted">(Optional)</small></label>
                                <input type="file" name="prev_document[]" class="form-control" accept="image/*,.pdf">



                                @if (!empty($prevDocuments[$index] ?? '' ))
                                <div class="mt-2">
                                    <img src="{{ asset($prevDocuments[$index] ?? '' ) }}" alt="Course Logo" width="80"
                                        class="rounded">
                                </div>
                                @endif
                            </div>
...
$(document).ready(function() {
    ...
    // =======================
    // Load subcourse details (mode + duration + eligibility)
    // =======================
    function loadSubCourseDetails(subCourseId) {
        ...
        $.get('/students/get-subcourse-details/' + subCourseId, function(data) {
            ...
            // Populate eligibility (previous qualifications) if not already present
            if(data.eligibility && data.eligibility.length > 0) {
                data.eligibility.forEach(function(qual) {
                    var exists = prevEduContainer.find('input[name="prev_qualification[]"]').filter(function() {
                        return $(this).val() == qual;
                    }).length;
                    if(!exists) {
                        var fieldHtml = `
                        <div class="row mb-4 p-3 border rounded">
