<div class="modal-body">
    ...
    <form id="subject-form" action="{{ route('subjects.update', $subject->id) }}" method="POST" class="row g-3">
        @csrf
        ...
        <div class="col-md-12">
            ...
            <select name="subcourse_id" id="subcourse_id" class="form-select" required>
                ...
                @foreach($subcourses as $subcourse)
                    <option value="{{ $subcourse->id }}" {{ $subject->subcourse_id == $subcourse->id ? 'selected' : '' }}>
                        {{ $subcourse->name }}
                    </option>
                @endforeach
            </select>
